...
\documentclass[brill,linguex]{glossa}
...
\title[Agreement Attraction in Turkish]{Agreement Attraction in Turkish\\ \bigskip \large Word count: \Sexpr{count}}
...
In the results section, we provide posterior distributions for parameters given our data and model with 95\% credible intervals. For our models, we run \Sexpr{n_chains} chains, each of which consists of \Sexpr{n_warmup} warm-up iterations and \Sexpr{n_iter-n_warmup} sampling iterations. Data for our study, along with our analysis scripts can be found \url{https://anonymous.4open.science/r/tr_short_agreement_attraction/}.
...
Figure \ref{fig:AverageResponses} shows the average proportions of `acceptable' responses by experimental condition for both the original experiment in \citet{LagoEtAl:2019} and our replication with unambiguous possessive marking. It shows that ungrammatical sentences with plural attractors are rated as acceptable more often (M = \Sexpr{exp1_cur_entry1$M}, SE = \Sexpr{exp1_cur_entry1$SE}) than their counterparts with singular attractors (M = \Sexpr{exp1_cur_entry2$M}, SE=\Sexpr{exp1_cur_entry2$SE}). The magnitude of the effect (\Sexpr{m_effect}) was in line with the findings reported in \citet{LagoEtAl:2019}, where the difference was \Sexpr{m_effect_lago}. Accuracy rates for grammatical conditions were nearly equal (M = \Sexpr{exp1_cur_entry3$M} and \Sexpr{exp1_cur_entry4$M}, SE = \Sexpr{exp1_cur_entry3$SE} and \Sexpr{exp1_cur_entry4$SE}, for singular and plural attractors respectively).

\begin{figure}[hbt!]
\centering

<<AverageResponses, echo=FALSE, fig.align='center', fig.height=3, dev='cairo_pdf', dev='png', dpi = 300, message=FALSE>>=
...
<<ResponseModel,  echo=FALSE, fig.align='center', fig.height=3, warning=FALSE, dev='cairo_pdf', dev='png', dpi = 300, message=FALSE>>=
...
The data that support the findings of this study, the code, and the paper resources including \LaTeX{} and \texttt{Sweave} files are openly available in the anonimized github page at \url{https://anonymous.4open.science/r/tr_short_agreement_attraction/}. %exclude
...
<<UngramResponseModel,  echo=FALSE, fig.align='center', fig.height=3, warning=FALSE, dev='cairo_pdf', dev='png', dpi = 300, message=FALSE>>=
...
library(knitr)
dat <- tables$results_table_ungram_responses
options(knitr.table.format = "latex")
@

<<results='asis', echo=FALSE, warning=FALSE, message=FALSE>>=
kable(dat, booktabs=TRUE, caption="Point estimate, estimate error, and credible interval for Regression coefficients")
@
